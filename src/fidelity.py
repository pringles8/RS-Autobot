...
def fid_buy_and_sell(driver, wait, buy=[], sell=[]):
    ...
    def exclusions():
        ...
        txt = wait.until(
            EC.element_to_be_clickable((By.XPATH, '//div[@class = "portfolio-card-container__banner"]'))).text
    ...
    def fid_modal(side):
        ...
        if side == "Buy":
            ...
            ask_price = wait.until(
                EC.visibility_of_all_elements_located((By.XPATH, '//div[@class="block-price-layout"]')))
        ...
        # Close Modal
        element = wait.until(
            EC.element_to_be_clickable((By.XPATH, '//a[@class = "float-trade-container-close dialog-close"]')))
        element.click()
    ...
    # Get all accounts
    accounts = wait.until(
        EC.presence_of_all_elements_located((By.XPATH, '//div[@class="acct-selector__acct-content"]')))
